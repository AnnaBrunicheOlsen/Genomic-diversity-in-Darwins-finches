...
vcftools --vcf population_SNPs.vcf --plink --out population_SNPs

# make test file with scaffold info instead of chromosome info
# take column with contig info
awk '{print $2}' population_SNPs.map > scaffold.txt
#split at the first full stop
awk -F'.' '{print $1}' scaffold.txt > scaffold2.txt
paste -d' ' scaffold2.txt population_SNPs.map > test.map
awk '{print $1,$3,$4,$5}' test.map > population_SNPs.map
...
# min 1KB in length
plink-1.9 --ped population_SNPs.ped --map population_SNPs.map --allow-extra-chr \
--genome
