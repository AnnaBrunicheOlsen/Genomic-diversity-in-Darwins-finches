...
vcffilter -f "DP > 15" ${LINE}_SNPs.vcf > out.vcf
...
cat header.txt body2.txt > ${LINE}_16x.vcf
...
vcftools --vcf ${LINE}_16x.vcf --plink --out ${LINE}_16x

# make test file with scaffold info instead of chromosome info
# take column with contig info
awk '{print $2}' ${LINE}_16x.map > scaffold.txt
#split at the first full stop
awk -F'.' '{print $1}' scaffold.txt > scaffold2.txt
paste -d' ' scaffold2.txt ${LINE}_16x.map > test.map
awk '{print $1,$3,$4,$5}' test.map > ${LINE}_16x.map
...
# min 1KB in length
plink-1.9 --ped ${LINE}_16x.ped --map ${LINE}_16x.map --allow-extra-chr \
--homozyg-snp 20 \
--homozyg-kb 10 --homozyg-window-het 0 --homozyg-window-snp 20 \
--homozyg-window-threshold 0.05 --out ${LINE}_16x

# count number of ROHs, ROHs are in column 9
awk '{print $9}' ${LINE}_16x.hom > ROH.txt
...
awk '{print $10}' ${LINE}_16x.hom > ROH_SNPs.txt
...
sed -i "s/sample1/$LINE/g" ${LINE}_16x.hom

# write results to file
cat ${LINE}_16x.hom >> /scratch/snyder/a/abruenic/Darwins_finches/individual_ROHs_16x.txt
...
# count heterozygotes
grep -a '0/1' ${LINE}_16x.vcf > het.txt
het="$(wc -l < het.txt)"

# count homozygotes
grep -a '0/0' ${LINE}_16x.vcf > homo.txt
grep -a '1/1' ${LINE}_16x.vcf >> homo.txt
homo="$(wc -l < homo.txt)"
